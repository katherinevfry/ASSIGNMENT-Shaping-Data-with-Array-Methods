...
const biggerThan = (element) => {
  return element >= 9000
}

businesses.forEach(business => {
  let orders = business.orders.some(biggerThan)
  if (orders === true) {
    document.querySelector('#spenders').innerHTML += `<h3>${business.companyName} orders more than $9000 in product</h3>`
  }
})
...
businesses.forEach(business => {
   business.orders.filter(order => {
    ...
    if (bigSpender){
      document.querySelector('#spenders').innerHTML += `<h3>${business.companyName} placed an order over $9000</h3>`
    }
  })
  })
...
*/
const printHere = document.querySelector("#planets");
printHere.innerHTML = "<h1>Planets</h1>";

planets.forEach(item => {
  printHere.innerHTML += `
  <h2>${item}</h2>`;

  printHere.innerHTML += "<hr/>"
...
planets.map(item => {
  printHere.innerHTML += `<h2>${item.charAt(0).toUpperCase()}${item.slice(1)}</h2>`;  
 });
...
const letterE = planets.filter(item => {
  let hasE = false;
...
letterE.forEach(item => {
  printHere.innerHTML += `<h2>${item}</h2>`;
})
...
const printThere = document.querySelector('#spam');
...
const emailAdds = contactInfo.map(item => {
  return item.email;
});

emailAdds.forEach(email => {
  email.forEach(i => {
    printThere.innerHTML += `<p>${i}</p>`
  })
});
...
document.querySelector('#nums').innerHTML = `<h1>${num}</h1>`
